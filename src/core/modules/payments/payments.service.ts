...
export class PaymentsService {
  ...
  async createPayment(payload: CreatePaymentDto, authUser: RequestWithUser['user']) {
  ...
  private async checkAccount(account: PaymeAccountRecord, amount: number) {
    ...
    const user = await this.prisma.user.findUnique({
      where: { id: parseInt(account.user_id), role: Role.STUDENT },
    });
    if (!user) {
      return {
        error: {
    ...
    if (pennyToAmount(amount).toString() !== course.price.toString()) {
      return {
        error: {
    ...
    return {
      result: {
        ...
        additional: {
          ...
          course_price: account.course.price.toString(),
